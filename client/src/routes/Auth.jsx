...
const Auth = () => {
  ...
  const login = async (e) => {
    try {
      ...
      let { success, message, orders, ...user } = await res.json();
  ...
  return (
    <form style={authStyles}>
      ...
      <span style={spanStyles}>
        <h3
          id="switch"
          onClick={action == 'login' ? () => toggleAction('signup') : () => toggleAction('login')}
          style={{ fontWeight: '100', cursor: 'pointer' }}
        >
          {action == 'login' ? 'New? Sign up here' : 'Back to login'}
        </h3>
        <button onClick={action == 'login' ? login : signup} style={buttonStyles}>
          {action}
          <ArrowRightIcon />
        </button>
      </span>
